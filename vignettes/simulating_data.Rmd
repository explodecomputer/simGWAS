...
This vignette demonstrates how to use the `sim_mv` function to simulate data for a few different types of GWAS summary data or individual level data. The design of the package implies that multiple phenotypes can be simulated simultaneously for a given causal relationship, they can have varying sample sizes and degrees of sample overlap, and the variants can have specified patterns of linkage disequilibrium.
...
+ `pi`: The proportion of SNPs that have a direct effect on each trait. This can be a scalar, a length $M$ vector, or a matrix with $J \times M$ dimension (more details on the matrix format can be found in the Effect Size Distribution vignette).
+ `G`: A matrix specifying direct causal effects between traits, on the scale of each trait having variance of 1. `G` should be an $M\times M$ matrix described below. Alternatively, if there are no causal effects between any traits, `G` can be given as the positive integer $M$. This is equivalent to `G = matrix(0, nrow = M, ncol = M)`. 
...
To generate a set of $M$ traits with no causal relationships, `G` can be set either equal to `M` or to an $M\times M$ matrix of 0's. Otherwise, `G` must be an $M \times M$ matrix with `G[i,j]` specifying the direct linear effect of trait $i$ on trait $j$. The diagonal entries of $G$ should be 0 (no self effects). An error will be generated if `G` specifies a graph that contains cycles. Since all traits have variance equal to 1, `G[i,j]^2` is the proportion of trait $j$ variance explained by the direct effect of trait $i$. 
...
colnames(G) <- rownames(G) <- c("X", "Y", "Z", "W")
G
```
...
By contrast, by allowing sporadic pleiotropy, some variants have direct effects on more than one trait

```{r}
sim_dat2a <- sim_mv(G = G,
                   J = 50000,
                  N = 60000,  
                  h2 = c(0.3, 0.3, 0.5, 0.4), 
                  pi = 1000/50000, 
...
sim_dat2a$Sigma_G
A <- data.frame(sim_dat2a$direct_SNP_effects_joint != 0) 
names(A) <- paste0("Direct ", c("X", "Y", "Z", "W"), " effect")
group_by_all(A) %>% 
  summarize(n = n()) %>% 
  arrange(-n)
...
#### Convenience functions for building G

Specifying causal relationships in a matrix can be slightly cumbersome so alternatively a text-based approach can be used e.g. reproducing the matrix above can be achieved using text-based syntax like this:
...
sim_dat2 <- sim_mv(G = G,
                  ...
                  h2 = c(0.3, 0.3, 0.5, 0.4), 
                  pi = 1, 
                  af = function(n){rbeta(n, 1, 1)})

sim_dat2$geno_scale
head(sim_dat2$snp_info)
```

In the default case there is no relationship between allele frequency and effect size

```{r}
with(sim_dat2, plot(beta_joint[,1] ~ snp_info$AF))
```

This represents a neutral selection model by default, which can be changed by specifying the selection parameter `S`

```{r}
sim_dat2a <- sim_mv(G = matrix(0, 4, 4),
                  N = 50000, 
                  J = 10000, 
                  h2 = c(0.3, 0.3, 0.5, 0.4), 
                  pi = 1, 
                  af = function(n){rbeta(n, 1, 1)},
                  S=c(0, 1.2, -1.2, 0))
for(i in 1:4) with(sim_dat2a, plot(beta_joint[,i] ~ snp_info$AF))
```
...
Note that 100k variants can be generated from an LD pattern for only 19k SNPs because `sim_mv` will automatically repeat the LD pattern to obtain the specified number of SNPs specified for `J`.
...
Note that there is no relationship between effects and allele frequencies by default

```{r}
for(i in 1:4) with(sim_dat1_LD, plot(beta_marg[,i] ~ snp_info$AF))
```

The selection parameter can be specified:

```{r}
sim_dat1_LDa <- sim_mv(G = G,
                      J = 1e5, 
                      N = 50000, 
                      h2 = c(0.3, 0.3, 0.5, 0.4), 
                      pi = 0.01, 
                      ...
                      af = AF, 
                      S=c(-2, 4, 0.5, 0))
for(i in 1:4) with(sim_dat1_LDa, plot(beta_joint[,i] ~ snp_info$AF))
```
