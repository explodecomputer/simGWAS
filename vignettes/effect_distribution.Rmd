...
Note also that there is no strong relationship between the allele frequency and the effect size (plotting below SNPs with non-zero effects)

```{r}
with(sim_dat1, plot(direct_SNP_effects_joint[beta_joint != 0] ~ snp_info$AF[beta_joint != 0]))
```

However, specifying the selection coefficient argument `S` is an option to draw effect estimates with respect to allele frequency, e.g. negative selection

```{r}
sim_dat1a <- sim_mv(G = 1,
                    J = 50000,
                   N = 0,  
                   h2 = 0.3, 
                   pi = 50000/50000, 
                   ...
                   S = 1)
with(sim_dat1a, plot(direct_SNP_effects_joint[beta_joint != 0] ~ snp_info$AF[beta_joint != 0]))
```

Or positive selection

```{r}
sim_dat1b <- sim_mv(G = 1,
                   J = 50000,
                   N = 0,  
                   h2 = 0.3, 
                   pi = 1000/50000, 
                   af = function(n){rbeta(n, 1, 5)},
                   S = 1)
with(sim_dat1b, plot(direct_SNP_effects_joint[beta_joint != 0] ~ snp_info$AF[beta_joint != 0]))
```
...
This can be a scalar in which case the probability is the same for all variants and all traits,
...
### Custom effect Distributions that Depend on Allele Frequency and Other Features
