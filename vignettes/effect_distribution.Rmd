...
with(sim_dat3, sum(beta_hat[,1]^2 * snp_info$AF * (1-snp_info$AF)))
with(sim_dat3, sum(beta_hat[,1]^2))

str(sim_dat3)
hist(sim_dat3$beta_hat)
with(sim_dat3, plot(abs(beta_hat) ~ snp_info$AF))
```
...
### Including selection coefficients

The mixture of normals can also be adapted to use the Bayes S method for relating allele frequencies and effect sizes by the selection coefficient S

$$
E(h^2_j) = x(2f_j(1-f_j))^S
$$


```{r}
set.seed(10)
myf <- mixnorm_to_scale_fam(sigma = c(1), pi = c(0.3), S=0.1, )
sim_dat8 <- sim_mv(G = 1,
                    J = 50000, 
                    N = 0, 
                    h2 = 0.3, 
